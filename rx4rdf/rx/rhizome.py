...
class Rhizome(object):
    ...
    def authorizeAdditions(self, additions, removals, reordered, user):
        ...
        forAllResource = self.server.domStore.dom.findSubject(RDF_SCHEMA_BASE+'Resource')
        ...
        for node in newPredicates:
            ...
            for s in newSubjects:
                current = self.server.domStore.dom.findSubject(s.uri)
                if current:
            ...
            for s in newResources:
                for p in s.childNodes:
                    if p.stmt.predicate == RDF_MS_BASE+'type':
                        classResource = self.server.domStore.dom.findSubject(p.stmt.object)
                        if classResource:
...
'''
    ...
    def processTemplateAction(self, resultNodeset, kw, contextNode, retVal):
        ...
        return self.server.callActions(actions, resultNodeset, kw, contextNode, retVal,
                    globalVars=self.server.globalRequestVars,errorSequence=errorSequence)
