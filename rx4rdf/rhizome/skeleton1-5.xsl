<?xml version="1.0"?>
<!-- 
Rule context are full XPath expressions, not XSLT patterns 
Implements ISO's let element (and if declared at top level, treat as params)
-->
...
<xsl:param name="previous:phase" />
<xsl:param name="phase">
  <xsl:choose>
    <xsl:when test="$previous:phase">
      <xsl:value-of select="$previous:phase"/>
    </xsl:when>
...
<xsl:template match="sch:schema | schema">
	<axsl:stylesheet version="1.0">
        ...
        <!-- implement the bit of the iso standard that we need --> 
         <xsl:for-each select="sch:let | let">
