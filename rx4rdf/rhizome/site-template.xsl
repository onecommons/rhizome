...
<label for="search" class="bold">Search</label><br/><input type="text" name="search" value="{$previous:search}" size="$edit-width" /><br/><label for="searchType">Type</label>&#xa0;<select name="searchType"><option value="Simple"><xsl:if test='not($previous:searchType) or $previous:searchType = "Simple"'><xsl:attribute name='selected'>selected</xsl:attribute></xsl:if>Simple</option><option value="RxPath" ><xsl:if test='$previous:searchType = "RxPath"'><xsl:attribute name='selected'>selected</xsl:attribute></xsl:if>RxPath</option><option value="RegEx" ><xsl:if test='$previous:searchType = "RegEx"'><xsl:attribute name='selected'>selected</xsl:attribute></xsl:if>Regex</option></select><br/><label for="view">View</label>&#xa0;&#xa0;<select name="view"><option value="list" selected='selected'>List</option><option value="summary">Summary</option><option value="rss20" >RSS 2.0</option><option value="rxml" >RxML</option><option value="rdf" >RDF/XML</option><option value="ntriples" >NTriples</option><option value="edit" >Edit</option><option value="rxpathdom" >RxPathDOM</option><option value="json" >JSON</option></select><br/><input type="submit" value="Search" name="Search"/></form></xsl:template>
...
<xsl:template name="actions-bar" >
  ...
  --><a rel='nofollow' href="site:///{$path}?action=edit{$aboutparam}" class="actionstab action-tab {f:if($action='edit', ' selected-action-tab selected-action-tab', '')}" title="Edit">Edit</a>&#xa0;<!--  
   --><a rel='nofollow' href="site:///{$path}?action=showrevisions{$aboutparam}" class="actionstab action-tab {f:if($action='showrevisions', ' selected-action-tab selected-action-tab', '')}" title="Revisions">Revisions</a>&#xa0;<!--  
